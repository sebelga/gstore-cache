...
const getKeys = (_keys, ...args) => {
    ...
    function onResult(_cacheResult) {
        ...
        if (cacheResult.length !== keys.length) {
            ...
            const keysNotFound = keys.filter(k => cached[dsKeyToString(k)] === undefined);
