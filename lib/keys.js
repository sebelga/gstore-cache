...
module.exports = cache => {
    ...
    const addKEYtoEntity = (entities, keys) =>
        entities.map((entity, i) => Object.assign({}, entity, { [ds.KEY]: keys[i] }));
    ...
    const wrap = (_keys, ...args) => {
        ...
        function onResult(_cacheResult) {
            ...
            const entities = isMultiple ? addKEYtoEntity(cacheResult, keys) : addKEYtoEntity(cacheResult, keys)[0];
            return [entities]; // wrap in an Array to align with google-cloud response
        }
    };
    ...
    const mset = (..._keysValues) => {
        ...
        if (keysValues.length === 2) {
            return cache.set(...keysValues);
        }
        return cache.mset(...keysValues);
    };
