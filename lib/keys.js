...
const getTTL = cache => {
    ...
    if (cache.config.stores.length > 1) {
        ttl = (data, storeName) => cache.config.ttl.stores[storeName].keys;
    } else {
        ttl = cache.config.ttl.keys;
    }
    return ttl;
};
...
module.exports = cache => {
    ...
    const wrap = (_keys, ...args) => {
        ...
        options.ttl = options.ttl || getTTL(cache);
        ...
        function onResult(_cacheResult) {
            ...
            if (cacheResult.length === 0) {
                ...
                return fetchHandler(keys).then(_fetchResult => {
                    ...
                    const fetchResult = orderEntities(arrify(_fetchResult), keys);
            ...
            if (cacheResult.length !== keys.length) {
                ...
                return fetchHandler(keysNotFound)
                    .then(_fetchResult => {
                        ...
                        const fetchResult = orderEntities(arrify(_fetchResult), keysNotFound);
